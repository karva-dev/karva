...
#[test]
fn test_env_var_isolation_between_tests() {
    // Env vars mutated directly (without monkeypatch) should not bleed into subsequent tests.
    let context = TestContext::with_file(
        "test.py",
        r"
import os
...
def test_sets_env_var():
    os.environ['_KARVA_TEST_ISOLATION'] = 'set'

def test_does_not_see_env_var():
    assert '_KARVA_TEST_ISOLATION' not in os.environ
        ",
    );
    ...
    ----- stdout -----
        Starting 2 tests across 1 worker
            PASS [TIME] test::test_sets_env_var
            PASS [TIME] test::test_does_not_see_env_var
