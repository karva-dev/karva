...
//!
//! The worker installs a Rust tracer (exposed to Python as a [`pyclass`])
//! using `sys.monitoring` on Python 3.12+ or `sys.settrace` on older
//! versions. It records every executed line under the configured source
//! roots. On stop, the worker computes executable lines for each touched
//! file by parsing it with [`ruff_python_parser`] and writes a per-worker
//! JSON file. The main runner combines those files into a terminal report.
...
#[derive(Default)]
struct TracerState {
    /// Filenames with the set of executed line numbers.
    executed: HashMap<PathBuf, HashSet<u32>>,
    /// Memoized result of [`should_track_path`] per filename string.
    track_cache: HashMap<String, Option<PathBuf>>,
}

#[pyclass(module = "karva", unsendable)]
...
#[pymethods]
impl CoverageTracer {
    /// `sys.monitoring` LINE event callback. Returns
    /// `sys.monitoring.DISABLE` for files outside the source roots so the
    /// interpreter stops calling us for them.
    fn line_cb(
        &self,
        py: Python<'_>,
        code: &Bound<'_, PyAny>,
    ...
    /// [`local_trace`] when the frame's file is under a source root.
    #[expect(
...
/// abandon a partial run; the data file is only persisted via
/// `stop_and_save`.
...
#[derive(Serialize)]
struct WriteOutput {
    files: BTreeMap<String, WriteFileEntry>,
}
...
fn save_data(
    ...
    let bytes = serde_json::to_vec(&WriteOutput { files })?;
    std::fs::write(data_file.as_std_path(), bytes)
}

/// Parse `path` and return the set of line numbers that contain a statement,
/// matching what coverage.py reports as the `Stmts` column.
fn executable_lines(path: &Path) -> HashSet<u32> {
